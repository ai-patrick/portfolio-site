...
(function () {
  ...
  // ── Config ──
  const CFG = {
    borderRadius: 14, borderWidth: 0.04, brightness: 60,
    opacity: 0.85, blur: 6, displace: 0, distortionScale: -60,
    redOffset: 0, greenOffset: 4, blueOffset: 8,
    xChannel: 'R', yChannel: 'G', mixBlendMode: 'difference'
  };

  // Browser detection — Safari & Firefox get fallback
  const isWebkit  = /Safari/.test(navigator.userAgent) && !/Chrome/.test(navigator.userAgent);
  const isFirefox = /Firefox/.test(navigator.userAgent);
  const svgDiv    = document.createElement('div');
  svgDiv.style.backdropFilter = 'url(#nav-glass-filter)';
  const svgSupported = !isWebkit && !isFirefox && (svgDiv.style.backdropFilter !== '' || svgDiv.style.webkitBackdropFilter !== '');
  if (!svgSupported) nav.classList.add('glass-fallback');

  const feImage   = document.getElementById('nav-fe-image');
  const dispRed   = { el: document.getElementById('nav-disp-red'),   offset: CFG.redOffset };
  const dispGreen = { el: document.getElementById('nav-disp-green'), offset: CFG.greenOffset };
  const dispBlue  = { el: document.getElementById('nav-disp-blue'),  offset: CFG.blueOffset };
  const feBlur    = document.getElementById('nav-fe-blur');

  // Dynamic displacement map generator
  function generateMap() {
    const rect = nav.getBoundingClientRect();
    const W = rect.width || 600, H = rect.height || 56;
    const edgeSize = Math.min(W, H) * (CFG.borderWidth * 0.5);
    // Red/Blue gradients mapping to displacement channels
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
      <defs>
        <linearGradient id="r" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0" stop-color="#ff0000"/>
          <stop offset="${edgeSize/W}" stop-color="#800000"/>
          <stop offset="${1 - edgeSize/W}" stop-color="#800000"/>
          <stop offset="1" stop-color="#000000"/>
        </linearGradient>
        <linearGradient id="g" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stop-color="#00ff00"/>
          <stop offset="${edgeSize/H}" stop-color="#008000"/>
          <stop offset="${1 - edgeSize/H}" stop-color="#008000"/>
          <stop offset="1" stop-color="#000000"/>
        </linearGradient>
      </defs>
  ...
  // Applies map + RGB offsets to filter elements
  function updateFilter() {
    if (!feImage) return;
    feImage.setAttribute('href', generateMap());
    [dispRed, dispGreen, dispBlue].forEach(({ el, offset }) => {
      if (!el) return;
      el.setAttribute('scale', String(CFG.distortionScale + offset));
      el.setAttribute('xChannelSelector', CFG.xChannel);
      el.setAttribute('yChannelSelector', CFG.yChannel);
    });
    if (feBlur) feBlur.setAttribute('stdDeviation', String(CFG.displace || 0.7));
  }

  // ResizeObserver regenerates map on navbar resize
  if (svgSupported) {
    new ResizeObserver(() => setTimeout(updateFilter, 0)).observe(nav);
  }
})();
